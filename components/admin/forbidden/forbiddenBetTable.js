...
import { getBetByForbiddenNumber, deleteBet } from "../../../lib/clientRequest/forbidden";
import { RiDeleteBin2Line } from "react-icons/ri";
import MyModal from "../../myModal";

export default function ForbiddenBetTable({forbidden}) {
  
  return (
    <>
      <div className="box-border my-5 mx-5 ">
        {forbidden.map((e) => (
          <Card forbidden={e} key={e._id} />
        ))}
      </div>
    </>
  );
...
const ModalDelete = ({ onClose, bet, forbidden }) => {
  ...
  const deleteMutation = useMutation(deleteBet, {
    onSuccess: (response) => {
     ...
     queryClient.prefetchQuery(["getBetByForbiddenNumber", forbidden._id],
     getBetByForbiddenNumber)
  ...
  return (
    <div className=" max-w-sm mx-auto my-10">
      ...
      <div className="flex flex-row justify-center gap-10 mt-5">
        <button className="w-28 border border-red-500 bg-red-500 rounded-md py-2"
          onClick={onClickDeleteHandler}
        >ลบ</button>
        <button className="w-28 border border-green-500 bg-green-500 rounded-md py-2"
